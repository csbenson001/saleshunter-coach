...
**Status: 1.1 on the App Store; 1.2 in preparation.** The app records in-person meetings, transcribes them live through the hosted relay, syncs them to the account, and ships a voice-typing keyboard extension. `CoachKit` holds the transcript core, ported line-for-line from the desktop's Rust with matching unit tests:
...
- `SonioxProtocol` + `SonioxStreamParser` — the Soniox realtime wire protocol as spoken through SalesHunter Coach's hosted STT relay (`wss://api.parley.tw/stt/stream`): config frame without vendor key, `keepalive`/`finalize` control frames, `<end>`/`<fin>` endpoint markers, error frames
...
```bash
cd ios/CoachKit && swift test     # core logic runs on macOS — no simulator needed
...
The app ships in **American English only**, same rule as the desktop
...
```
App/SalesHunter Coach/Localizable.xcstrings     App/SalesHunter Coach/InfoPlist.xcstrings
...
**not** localize). The app ships **American English only**: `knownRegions` in
`project.yml` lists `en` alone, and no second locale belongs in the catalogs.
...
Driven by a `#if DEBUG` demo mode (`App/SalesHunter Coach/ScreenshotDemo.swift`) that serves
fictional fixtures and is navigated by `saleshunter-coach://demo/…` URLs — no review
