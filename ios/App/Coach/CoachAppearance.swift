...
///
/// This is the one place that fixes them. `CoachApp` calls `apply()` once at
/// launch; nothing else should touch a UIKit appearance proxy.
///
/// Two rules the rest of the file follows:
///
/// - **Colours are built from `CoachDesignTokens` directly**, as
...
enum CoachAppearance {
    ...
    private static func applyNavigationBar() {
            ...
            -> UINavigationBarAppearance
        {
            ...
            a.largeTitleTextAttributes = [
                .font: scaledFont(CoachTypography.Face.bold, 34, .largeTitle, fallback: .bold),
                .foregroundColor: foreground,
            ]
            a.titleTextAttributes = [
                .font: scaledFont(
                    CoachTypography.Face.semibold, 17, .headline, fallback: .semibold),
                .foregroundColor: foreground,
            ]
            ...
            let buttonFont = scaledFont(
                CoachTypography.Face.regular, 17, .body, fallback: .regular)
    ...
    private static func applyTabBar() {
        ...
        let title = scaledFont(CoachTypography.Face.medium, 10, .caption2, fallback: .medium)
        for item in [
    ...
    // MARK: - Colours
    //
    // Read from CoachDesignTokens, never re-declared here.

    private static let foreground = dynamic(
        CoachDesignTokens.Light.foreground, CoachDesignTokens.Dark.foreground)
    private static let mutedForeground = dynamic(
        CoachDesignTokens.Light.mutedForeground, CoachDesignTokens.Dark.mutedForeground)
    /// `Theme.brand` in light, `Theme.sky` in dark: #1469D4 is too dark to read
    /// as a selected state on the navy-black page.
    private static let accent = dynamic(CoachDesignTokens.brand, CoachDesignTokens.sky)
    ...
    private static let separator = dynamic(
        CoachDesignTokens.Light.border, CoachDesignTokens.Dark.border, darkAlpha: 0.10)
    ...
    /// resources have drifted apart (`CoachTypography.unresolvedFaces()`
