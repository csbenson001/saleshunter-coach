    ...
    var body: some View {
        NavigationStack {
            ...
            .alert("Before you start recording", isPresented: $showRecordingConsent) {
                ...
                Text("Coach picks up the room through the microphone, sends the audio to your Coach account for live transcription, and syncs the recording and transcript there. Confirm that everyone present has agreed to be recorded.")
            }
            #if DEBUG
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 4) {
            Text(verbatim: TranscriptClipboard.liveLabel(for: segment))
                .font(.coach.caption2.weight(.semibold))
                .foregroundStyle(
                    segment.speaker == 0
...
struct LevelMeter: View {
    ...
    // under the `-CoachDemo` launch argument.
    #Preview("Live — light") {
