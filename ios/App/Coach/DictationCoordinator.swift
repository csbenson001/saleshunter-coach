...
/// keyboard extension can't record, so it opens `saleshunter-coach://dictate`, this object
...
final class DictationCoordinator: ObservableObject {
    ...
    /// Start from the keyboard's `saleshunter-coach://dictate?session=…`.
    ...
    /// so mint one and publish it for whichever Coach keyboard is frontmost.
    ...
    private func launch() async {
        ...
        guard let token = KeychainStore.get(AppState.tokenKey) else {
            fail(String(localized: "Sign in to the Coach app before using the voice keyboard."))
            return
        }
        ...
        guard await AudioCapture.requestPermission() else {
            fail(
                String(
                    localized:
                        "Dictation needs microphone access. Turn it on in Settings › Coach."))
            return
        }
    ...
    ///     the keyboard only falls back to `saleshunter-coach://dictate` when this note
    ...
    /// user came back to Coach itself rather than bouncing to a host app).
    func dismiss() async {
        if active { await stop() }
        active = false
    }
        ...
        /// ScreenshotDemo (`saleshunter-coach://demo/dictation`): the stranded-listening
