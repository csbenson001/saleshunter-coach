...
struct SettingsView: View {
    ...
    var body: some View {
        NavigationStack {
            ...
            .confirmationDialog(
                ...
                Text("This permanently removes your Coach account, personal recordings, transcripts, folders, and usage data. Organizations you still own have to be transferred or deleted first.")
            }
        }
    }
    ...
    private func sectionHeader(_ title: LocalizedStringKey) -> some View {
        Text(title)
            .font(.coach.footnote.weight(.semibold))
            ...
            if let user = app.user {
                HStack(spacing: 12) {
                    ...
                    VStack(alignment: .leading, spacing: 2) {
                        Text(verbatim: user.name ?? user.email).font(.coach.bodyEmphasized)
                        Text(verbatim: user.email).font(.coach.caption)
                            .foregroundStyle(Theme.mutedForeground)
                    }
                }
                .padding(.vertical, 4)
    ...
    private var signInForm: some View {
        ...
        if let err = app.signInError {
            Text(verbatim: err).font(.coach.caption).foregroundStyle(Theme.destructive)
        }
        Text("Opens Coach's sign-in page — email and password, Google, and Apple. Once you're in you get live transcription with no API key, plus recording and transcript sync.")
            .font(.coach.caption)
            .foregroundStyle(Theme.mutedForeground)
    }
        ...
        Circle()
            ...
            .overlay(
                Text(String((user.name ?? user.email).prefix(1)).uppercased())
                    .font(.coach.callout.weight(.semibold))
                    .foregroundStyle(Theme.primary))
    }
    ...
    private func quotaBar(label: String, used: Double, limit: Double, unit: String) -> some View {
        VStack(alignment: .leading, spacing: 8) {
            HStack {
                Text(verbatim: label).font(.coach.subheadlineEmphasized)
                Spacer()
                Text(verbatim: String(format: "%.1f / %.0f %@", used, limit, unit))
                    .font(.coach.caption.monospacedDigit())
                    .foregroundStyle(Theme.mutedForeground)
            }
        ...
        Section {
                ...
                HStack(spacing: 14) {
                    ...
                    VStack(alignment: .leading, spacing: 3) {
                        Text("Type by voice in any app")
                            .font(.coach.headline)
                        Text("Tap the mic on the Coach keyboard and your words land at the cursor.")
                            .font(.coach.caption)
                            .foregroundStyle(Theme.mutedForeground)
            ...
            DisclosureGroup {
                dictationStep(
                    ...
                    detail: "In Keyboards → Add New Keyboard, pick Coach Voice.")
                dictationStep(
                    number: 2, title: "Allow Full Access",
                    detail: "Lets your voice reach your Coach account to be transcribed.")
                dictationStep(
                    number: 3, title: "Action Button (optional)",
                    detail: "Map it to Coach Voice Typing to start dictation without switching keyboards.")
            } label: {
                Text("Set-up steps").font(.coach.subheadlineEmphasized)
            }
        } header: {
        ...
        -> some View
    {
        HStack(alignment: .top, spacing: 12) {
            Text(number, format: .number)
                .font(.coach.caption.weight(.bold).monospacedDigit())
                .foregroundStyle(Theme.onBrand)
                .frame(width: 24, height: 24)
                .background(Circle().fill(Theme.brandGradient))
            VStack(alignment: .leading, spacing: 3) {
                Text(title).font(.coach.subheadlineEmphasized)
                Text(detail).font(.coach.caption).foregroundStyle(Theme.mutedForeground)
                    .fixedSize(horizontal: false, vertical: true)
            }
        }
        .padding(.vertical, 5)
    ...
    private var aboutSection: some View {
        Section {
            ...
            Link("SalesHunter Coach for Mac", destination: CoachLinks.site)
            Link("Privacy Policy", destination: CoachLinks.privacy)
            Link("Support & feedback", destination: CoachLinks.support)
        } footer: {
