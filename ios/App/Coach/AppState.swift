...
final class AppState: NSObject, ObservableObject {
    ...
    /// back to `saleshunter-coach://` with the session token. Credentials never pass
    /// through the app.
    func signIn() {
        signInError = nil
        signingIn = true
        let url = cloud.signInURL(callback: "saleshunter-coach://auth/cb")
        let session = ASWebAuthenticationSession(url: url, callbackURLScheme: "saleshunter-coach") {
    ...
    /// Turn the `saleshunter-coach://auth/cb?token=…` handoff into a live session.
    ///
