...
struct LibraryView: View {
    ...
    var body: some View {
        NavigationStack {
            ...
            // `saleshunter-coach://demo/transcript` pushes the demo recording, so the
    ...
    private var scopeMenu: some ToolbarContent {
        ToolbarItem(placement: .topBarTrailing) {
            Menu {
                ...
                .font(.coach.subheadlineEmphasized)
    ...
    private func chip(_ label: String, selected: Bool, action: @escaping () -> Void) -> some View {
        Button(action: action) {
            Text(verbatim: label)
                .font(.coach.caption.weight(selected ? .semibold : .regular))
...
private struct RecordingCard: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 7) {
            ...
            .font(.coach.caption2.monospacedDigit())
            .lineLimit(1)
    ...
    /// from `ScreenshotDemo` when the app is launched with `-CoachDemo`, which
