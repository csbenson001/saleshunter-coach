...
/// The shared channel between the Coach keyboard extension and the container
...
public enum DictationChannel {
    ...
    public static let appGroup = "group.com.saleshunter.coach.ios"

    /// app → keyboard: the transcript grew or the state changed.
    public static let downNote = "com.saleshunter.coach.dictation.down"
    ...
    public static let upNote = "com.saleshunter.coach.dictation.up"
    ...
    public static func startURL(session: String) -> URL {
        URL(string: "saleshunter-coach://dictate?session=\(session)")!
    }

    public static func session(fromStart url: URL) -> String? {
        guard url.scheme == "saleshunter-coach", url.host == "dictate",
