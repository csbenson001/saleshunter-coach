...
- **一句話**：讓 SalesHunter Coach 從「單場會議的教練」長出「跨會議的客戶戰情系統」——一個**從通話裡自己長出來的 CRM**，而且戰情會反過來餵回 live 教練（紅線守衛）。

---

## 1. 動機

業務實際的工作單位不是「一場會議」，是「一家公司裡的一群人、和幾條同時推進的局」。目前 SalesHunter Coach 每場會議是一筆獨立的 `HistoryEntry`，「這場跟誰開、屬於哪個案子、上次漏問什麼」全靠人腦。同時，業務手上真正值錢的是戰局分析（人物立場、籌碼對比、風險紅線、下一步槓桿順序——見附錄 A 的兩個真實範例形態），這種分析今天只能散落在筆記軟體裡，會過期、有錯誤、不可查證。

CRM 的死因是要人手動餵資料。SalesHunter Coach 的不對稱優勢是**它擁有最富的資料源（全程逐字稿）**：會後自動萃取、逐項審核入庫，資料保鮮問題從根本上被繞過。

## 2. 決策記錄（三輪討論拍板）

| # | 決策 | 內容 |
|---|------|------|
| D1 | 定位 | **A-lite**：SalesHunter Coach 自足的 mini-CRM（不是外部 CRM 的附庸），本機優先；schema 預留 `externalCrmId`，未來可單向推 Twenty/HubSpot |
...
| D11 | Onboarding | 貼逐字稿 → 走 upload 通道成正式會議＋抽卡；雜項筆記 → 公司附件、只抽卡（MVP）。**訪談模式**（SalesHunter Coach 反問你補缺口）= 第二輪 |
...
第二輪加**訪談模式**：SalesHunter Coach 反問（「決策者是誰？」「有競品了嗎？」）補缺口——本質是缺口引擎在會前跑，先讓貼資料這條路順。
...
- 主觀：Brandon 能在 SalesHunter Coach 裡維護 AI3／喜來登等級的戰局分析，不再另開筆記軟體
