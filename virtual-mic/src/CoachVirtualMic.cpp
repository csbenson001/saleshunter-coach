// Coach Microphone — a macOS virtual audio device (AudioServerPlugIn / HAL).
//
...
// "Coach Microphone" as their microphone. Coach's live-translation pipeline
...
// is writing (Coach idle) the input goes silent instead of looping stale audio.
...
std::shared_ptr<aspl::Driver> CreateCoachDriver()
{
    auto context = std::make_shared<aspl::Context>();

    aspl::DeviceParameters params;
    params.Name = "SalesHunter Coach Microphone";
    params.Manufacturer = "Pathors";
    // Stable UID so apps remember it as the selected device across restarts.
    params.DeviceUID = "com.saleshunter.coach.virtualmic:0";
    params.ModelUID = "com.saleshunter.coach.virtualmic";
...
extern "C" void* CoachVirtualMicEntryPoint(CFAllocatorRef /*allocator*/, CFUUIDRef typeUUID)
{
    if (!CFEqual(typeUUID, kAudioServerPlugInTypeUUID)) {
        return nullptr;
    }
    // Keep the driver alive for the lifetime of the host (coreaudiod).
    static std::shared_ptr<aspl::Driver> driver = CreateCoachDriver();
