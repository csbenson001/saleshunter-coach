# SalesHunter Coach Microphone — virtual audio driver

A macOS **AudioServerPlugIn** (HAL) virtual audio device named **“SalesHunter Coach
...
Teams…) can select **SalesHunter Coach Microphone** as their microphone.

This is what lets SalesHunter Coach’s live translation reach a meeting: the translate
pipeline plays the translated speech into this device, and the meeting app hears
it as the user’s mic.

```
SalesHunter Coach (translate) ──play──▶ [SalesHunter Coach Microphone] ──appears as mic──▶ Google Meet
...
([`src/CoachVirtualMic.cpp`](src/CoachVirtualMic.cpp)) is just the two realtime
I/O callbacks plus one output + one input stream sharing a ring buffer. No
third-party runtime dependency — the driver is entirely SalesHunter Coach’s own.
...
```bash
./build.sh                  # → build/CoachMicrophone.driver (arm64, host arch)
...
```bash
./make-pkg.sh        # → build/CoachMicrophone.pkg
```
...
BlackHole. The bundled `postinstall` reloads `coreaudiod` so **SalesHunter Coach
...
see **SalesHunter Coach Microphone**.

Uninstall (either path):

```bash
sudo rm -rf "/Library/Audio/Plug-Ins/HAL/CoachMicrophone.driver" && sudo killall coreaudiod
```

## Using it with SalesHunter Coach

Once installed, **SalesHunter Coach Microphone** shows up as a normal output device, so the
Live Translation window’s **output-device picker lists it automatically** (no app
change needed). Select it there, then in Google Meet pick **SalesHunter Coach Microphone**
...
1. **CI** (`.github/workflows/release.yml`, step “Build SalesHunter Coach Microphone
   ...
   `src-tauri/virtualmic/CoachMicrophone.pkg` — which `tauri.conf.json`
   bundles into `SalesHunter Coach.app` as a resource.
2. **In-app one-click install** (`src-tauri/src/virtual_mic.rs` + the install
   card in the Live Translation window): when the device is absent, SalesHunter Coach
   ...
   “SalesHunter Coach Microphone” appears and auto-selects it as the output device.
...
4. **Dev builds**: `build.rs` writes a zero-byte placeholder so tauri-build
   ...
   to `virtual-mic/build/CoachMicrophone.pkg` (build it with the scripts
