package com.saleshunter.coach.auth
...
import com.saleshunter.coach.cloud.CloudClient
import com.saleshunter.coach.cloud.CoachHttp
 ...
 * `filesDir/datastore/coach_auth.preferences_pb`, which is app-private
 ...
 */
private val Context.coachAuthStore: DataStore<Preferences> by preferencesDataStore(name = "coach_auth")

/** Outcome of feeding a `saleshunter-coach://` deep link to [AuthManager.handleAuthCallback]. */
 ...
 * is addressed, and how the `saleshunter-coach://auth-callback?token=…` hand-off turns into
 * a live session.
 *
 ...
 * 3. The page 302s to `saleshunter-coach://auth-callback?token=…`, which `MainActivity`
...
class AuthManager(
    ...
    private val http: OkHttpClient = CoachHttp.shared,
) {
    private val store = context.applicationContext.coachAuthStore
     ...
     * `saleshunter-coach://` target or an http loopback, falling back to
     * `saleshunter-coach://auth-callback` for anything else.
     *
    ...
    companion object {
        /** Matches the `coach` / `auth-callback` intent filter in AndroidManifest.xml. */
        const val CALLBACK_SCHEME = "saleshunter-coach"
        const val CALLBACK_HOST = "auth-callback"

        /** iOS uses `saleshunter-coach://auth/cb`; accepted too so one token can serve both. */
